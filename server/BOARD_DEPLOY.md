...
## D. 마스터 스위치 — Title Data `web_board`

일반 **Title Data**(Internal 아님) 키 `web_board` 로 게시판 전체를 켜고 끕니다.

| 값 | 상태 |
|---|---|
| `1` / 키 없음 / 조회 실패 | **활성** (기본값, fail-open) |
| `0` / `false` / `off` | 비활성 |

**비활성일 때:**
- 랜딩에서 게시판 섹션이 **통째로 사라집니다** (글 조회조차 하지 않음)
- 글쓰기(`writeBoardPost`) 차단
- 원탭 토큰 발급(`issueWebToken`) 차단

**비활성이어도 유지되는 것:**
- 이미 올라간 글은 삭제되지 않습니다 — 다시 켜면 그대로 복귀합니다
- 기존 세션키도 살아 있습니다(재연결 불필요)
- 앱의 💬 버튼은 계속 보입니다. 눌러도 게시판이 없는 랜딩이 열릴 뿐입니다
  (버튼 노출은 스위치로 제어하지 않습니다 — 앱 재배포 없이 서버만으로 끄는 게 목적이므로)
- `?wt=` 토큰은 교환하지 않고 주소창에서 제거만 합니다 → 헛되이 소각되지 않습니다
...
신고 대응 도구는 [`admin_board.ps1`](admin_board.ps1) — 조회 / 글·작성자 단위 삭제 / 웹 세션 차단.
사용법과 상황별 대응은 **[`ADMIN_TOOLS.md`](ADMIN_TOOLS.md)** 에 정리되어 있다.
