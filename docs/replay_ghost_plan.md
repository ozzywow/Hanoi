...
> Tower of Hanoi - Speedrun — 터치노트 기록/재생 및 랭커 고스트 대결 기능
...
### 3차 — 고스트 대결 & 랭킹 쟁탈
**화면 방식 확정(2026-07-10): 상대 보드 렌더 없음 → "고스트 레이스 HUD"** (분할/PiP 폐기).
- 내 보드는 **풀 화면 라이브 플레이** 그대로, 상단/하단에 진행 바 + 실시간 ±시간 델타만.
- 상세 설계는 **§10** 참조.
...
| D6 | ✅ 3차 대결 상대 매칭 | **특정 랭커 지목 (카드 RACE 버튼)** | 관전 카드에서 직접 도전 |
| D10 | ✅ 3차 화면 방식 | **고스트 레이스 HUD (진행바+±델타)** | 분할/PiP 폐기 (§10) |
| D11 | ✅ 3차 진행도 지표 | **이동 남은 수 (distance-to-solved)** | 매 수 갱신+공정, 디스크확정/이동수 폐기 (§10.2) |
| D12 | 3차 레이스 종료 | 완주까지 진행(고스트 골인 순간 연출) vs 고스트 골인 시 종료 | §10, 완주 권장 |
...
## 10. 3차 상세 — 고스트 레이스 (확정 2026-07-10)

### 10.1 프레이밍
- **일반 타임어택 라이브 플레이 + 고스트 HUD + 고스트-인식 결과화면.** 별도 "레이스 모드 서버 로직" 없음.
- **랭킹 쟁탈 = 기존 리더보드가 알아서** — 고스트 기록보다 빠르면 정상 제출로 위에 랭크됨. HUD와 결과화면이 이걸 "빼앗았다"고 극적으로 연출.
- 상대 보드는 **안 그림.** 경쟁 정보는 "지금 앞서나/뒤처지나" 스칼라 하나. (레이싱 게임 고스트 방식)

### 10.2 진행도 지표 (D11 ✅ = **이동 남은 수 / distance-to-solved**)
- **진행도 = 현재 보드에서 완성까지 최소 이동 수 `R` (0 = 완성, 시작 = 2^N−1).**
  - ⚠️ "디스크 확정 수"는 폐기 — 최대 디스크가 **한가운데(2^(N-1)수)에야** 목표에 도착해서 **전반부 내내 P=0** → 델타가 `ghostReach[0]=0` 때문에 `−myT`로 **가짜 뒤처짐**을 표시(모든 레벨의 전반부가 죽음).
  - `R`은 **매 수마다 변함**(촘촘) + **상태만의 함수라 공정**(이동 몇 번 했는지 무관) + 시작 둘 다 2^N−1(대등). blunder하면 `R` 증가 → 뒤처짐.
- **계산(O(N))** — 목표 폴 `T`까지 최소 이동:
  ```
  R(state, T):  t=T; c=0
    for k=N..1:            # 큰 디스크부터
       p = 디스크 k의 폴
       if p != t: c += (1<<(k-1)); t = 3 - p - t   # 나머지 폴(합=3)
    return c
  ```
  검증(L3 시작)=7 → 최적 1수 후 6 → … → 0. 매 수 1 감소.

### 10.3 고스트 도달시각 테이블 (precompute)
- 고스트 blob 디코드 → 가상 보드로 MOVE_OK 순차 적용, 각 이벤트 후 `R` 계산.
- `ghostReach[r]` = 고스트가 **처음 남은수 ≤ r 이 된 t_ms** (min-so-far 추적, 회귀 무시). `r`은 0..2^N−1.
- `ghostReach[0]` = 완주 시간 = 랭커 `scoreMs`. 1패스, 저비용. (배열 크기 2^N: L10=1024, 무시 가능)

### 10.4 라이브 & 델타
- 내 이동 확정([AttachDiscusToPole](../Classes/PlayScene.cpp#L992)) 후 내 남은수 `R_me` 계산(내 `m_pole`), 내 경과 `myT = getMilliCount()-m_dateTime`.
- **델타 = `ghostReach[R_me] - myT`** → 양수면 같은 지점을 고스트보다 **빨리** 도달 = **AHEAD**, 음수면 BEHIND.
- 매 수 `R_me`가 바뀌므로 **처음부터 끝까지 촘촘하게 갱신**. 표시: `AHEAD +1.4s`(초록)/`BEHIND −0.9s`(빨강).

### 10.5 HUD
- **진행 바 + 두 마커**: 채움 = `(2^N−1 − R)/(2^N−1)`. YOU = 내 `R_me` 기준, GHOST = 실시간 `t`에서 `ghostReach[r]<=t`인 최소 `r` 기준(시간 따라 상승).
- 델타 라벨(위). 기존 HUD/드로우 유틸 재사용, **보드 렌더 0**.
- 고스트가 완주(=scoreMs 경과) 순간 "GHOST FINISHED" 연출(D12).

### 10.6 흐름
1. **진입**: 관전 카드([showAwardCardDialog](../Classes/MainScene.cpp#L1540))에 ▶WATCH 옆 **⚔ RACE** 버튼 → `createRaceScene(level, ghostBlob, name, rank, scoreMs)`. (D6: 특정 랭커 지목)
2. 일반 카운트다운 → GO. 내 라이브 플레이. 고스트 HUD 동작.
3. **완주**([Finished](../Classes/PlayScene.cpp#L560)): `myT_final < scoreMs`면 승 → 정상 `submitScore`로 순위 교체 + "OO 님을 이겼다! RANK n" 결과화면. 지면 기록은 그대로 내 기록으로 제출(D12: 완주까지 진행).

### 10.7 재사용/신규
- **재사용**: 라이브 게임 엔진 전체(관전과 달리 진짜 플레이), 고스트 blob 페치(WATCH와 공유), 진행도용 `m_pole`.
- **신규**: 진행도 함수, ghostReach 테이블, HUD(바+마커+델타), DrawTime에 고스트 마커/델타 갱신 훅, RACE 버튼+createRaceScene, 고스트-인식 결과화면.
- 규모: 중간(대부분 HUD 드로잉). 보드 렌더가 없어 분할/PiP보다 훨씬 저비용.

### 10.8 미해결 (D12 외)
- ~~델타 granularity~~ → **해결**: 진행도를 "이동 남은 수"로 바꿔 매 수 갱신(§10.2).
- 무결성: 3차는 내 라이브 기록이라 조작 위험은 기존 스코어 제출과 동일(고스트는 읽기전용). 별도 서버검증 불필요(R3는 "고스트가 조작 기록"인 경우인데, 업로드측 §5.3 서버 Top10 검증으로 이미 커버).

---

## 11. 백로그 (3차 이후)

### 11.1 디스크 확정 마일스톤 이펙트 (게임 필 개선)
- 레이스 진행도로는 폐기된 **디스크 확정 P**(목표 폴 바닥부터 확정 디스크 수)를 **긍정 피드백**으로 재활용.
- **트리거**: 라이브 플레이 중 P 증가 순간(이동 확정 후 P 재계산). **마일스톤 = P 1~N−1**(P=N은 이미 클리어).
- **이펙트**: 안착 디스크/목표 폴 글로우·파티클·플래시 + "잠김" 효과음. 뒤 디스크일수록 점층. 최대 디스크 안착(=절반 돌파)은 특히 강조.
- 일반 플레이 전반에 적용(3차 레이스에도 얹힘). P 계산은 R과 별개로 간단(목표 폴 바닥부터 연속 카운트).
- 착수 시점: **3차 구현 완료 후.**
